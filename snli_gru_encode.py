...
def main(num_epochs=NEPOCH):
    ...
    #premise encode
    l_prem_encode1 = lasagne.layers.GRULayer(l_prem_embed, GRUENCODE, mask_input=l_mask_p)
    l_prem_encode1_dpout = lasagne.layers.DropoutLayer(l_prem_encode1, p=DPOUT, rescale=True)
    l_prem_encode2 = lasagne.layers.GRULayer(l_prem_encode1_dpout, GRUENCODE, mask_input=l_mask_p)

    # ATTEND
    l_hypo_embed_dpout = lasagne.layers.DropoutLayer(l_hypo_encode2, p=DPOUT, rescale=True)
    ...
    l_prem_embed_dpout = lasagne.layers.DropoutLayer(l_prem_encode2, p=DPOUT, rescale=True)
