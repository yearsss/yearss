...
from util_layers import (DenseLayer3DInput,
...
def main(num_epochs=NEPOCH):
    ...
    l_hypo_gru1 = GRULayer(l_hypo_comphid2, GRUHID, mask_input=l_mask_h)
    l_hypo_gru1_dpout = lasagne.layers.DropoutLayer(l_hypo_gru1, p=DPOUT, rescale=True) 
    l_hypo_gru2 = GRULayer(l_hypo_gru1_dpout, GRUHID, mask_input=l_mask_h)
    l_hypo_gru = SliceLayer(l_hypo_gru2,-1,1)
	
    l_prem_gru1 = GRULayer(l_prem_comphid2, GRUHID, mask_input=l_mask_p)
    l_prem_gru1_dpout = lasagne.layers.DropoutLayer(l_prem_gru1, p=DPOUT, rescale=True)
    l_prem_gru2 = GRULayer(l_prem_gru1_dpout, GRUHID, mask_input=l_mask_p)
    l_prem_gru = SliceLayer(l_prem_gru2,-1,1)

    l_hypo_mean = lasagne.layers.DropoutLayer(l_hypo_gru, p=DPOUT, rescale=True)
    l_prem_mean = lasagne.layers.DropoutLayer(l_prem_gru, p=DPOUT, rescale=True)
    ...
    try:
        for epoch in range(num_epochs):
            ...
            cPickle.dump(all_param_values,
                         open('/data1/data/hansongbo/parikh_snli-master/params' + os.sep + 'params_' + filename + '.pkl', 'wb'))
