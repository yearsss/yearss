...
'''
    2016.11.6
    use lstm to compare the every pair of word and context word
'''
...
def main(num_epochs=NEPOCH):
    ...
    l_lstm_h = LSTMLayer(l_hypo_premwtd_dpout,num_units=LSTMHID,mask_input=l_mask_h)
    l_comp_h = SliceLayer(l_lstm_h,-1,1)
    
    l_prem_hypowtd_dpout = lasagne.layers.DropoutLayer(l_prem_hypowtd, p=DPOUT, rescale=True)
    l_lstm_p = LSTMLayer(l_prem_hypowtd_dpout,num_units=LSTMHID,mask_input=l_mask_p)
    l_comp_p = SliceLayer(l_lstm_p,-1,1)

    # AGGREGATE
    # output dim: (BSIZE, 4*LSTMHID)

    l_v1v2 = lasagne.layers.ConcatLayer([l_comp_h, l_comp_p], axis=1)
